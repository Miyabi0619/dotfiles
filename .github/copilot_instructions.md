...
-   `dot_config/`: Directory for XDG Base Directory Specification compliant configurations.
    ...
    -   `zsh/`: Directory for Zsh configurations. The templates contain separate considerations for Linux and macOS, and should be adapted per-OS:
        -   Linux: `dot_zshrc.tmpl` (Linux-first configuration) and `dot_zshenv.tmpl`.
        -   macOS: Mac placeholders exist in the templates and should be expanded for macOS-specific settings (e.g., Homebrew paths, macOS-only plugins).
        -   `plugins/`: Placeholder for Zsh plugin lists. Use OS-specific lists as required and ensure `sheldon` consumes the correct list for the host OS.
    -   `sheldon/`: Directory for Sheldon plugin manager configuration. Maintain OS-specific plugin lists or templates where needed:
        -   Linux: `plugins.toml.tmpl` configured for typical Linux plugin sets.
        -   macOS: Add/adjust `plugins.toml.tmpl` to include macOS-specific plugin choices if required.
...
### 2.1. Immediate Next Steps (Linux & macOS)

1.  **Test Zsh Environment (Linux)**:
    -   Although `zsh` and `sheldon` are installed, and configurations are in place, the user's default shell remains `bash` on Linux.
    -   The user should manually launch `zsh` from `bash` to verify that `starship` (prompt), `sheldon` (plugins), and all `zshrc` settings (e.g., history, options) are functioning as expected on Linux.
    -   Adjust `dot_zshrc.tmpl`, `plugins.toml.tmpl`, and `starship.toml.tmpl` as needed based on testing feedback.
2.  **Test Zsh Environment (macOS)**:
    -   For macOS, check and expand the macOS placeholders in `dot_zshrc.tmpl` and `starship.toml.tmpl`. Confirm that OS-specific paths, Homebrew locations, and macOS plugin choices for `sheldon` are set correctly.
    -   Validate that `sheldon` loads the appropriate macOS plugin list and that `zsh` settings are compatible with macOS-specific shell behaviors.
3.  **User's Existing Dotfiles Review**:
